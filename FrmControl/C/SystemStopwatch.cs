...
namespace FrmControl.C
{
	public class SystemStopwatch: UserControl
	{
		...
		private void StartAnewStopwatch()
		{
			...
			bool r = Task.WaitAll(new Task[] { ta },5000);
		...
		{
			 DataTarget target = DataTarget.AttachToProcess(processId, 500000, AttachFlag.Passive);
			System.Threading.Thread.Sleep(400); // 等待数据收集

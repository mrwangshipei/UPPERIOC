     ...
     /*   private void GetInstance()
        ...
        public object InitInstance(Type item,bool SubRegister = false,string name = null)
        {
            ...
            try
            {
				...
				cos = item.GetConstructors().ToList().FirstOrDefault(item => item.GetCustomAttribute(typeof(IOCConstructor)) != null) == null ? item.GetConstructors()[0] : item.GetConstructors().ToList().FirstOrDefault(item => item.GetCustomAttribute(typeof(IOCConstructor)) != null);
			}
            catch (Exception)
            {
